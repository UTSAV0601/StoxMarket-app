...
import {
  Box, Typography, Grid, CircularProgress, Card, CardContent, Button, IconButton,
} from "@mui/material";
...
function StockDetail() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (!historyRes.data || !Array.isArray(historyRes.data.values) || historyRes.data.values.length === 0) {
          setChartData(null);
          return;
        }
  ...
  const handleWatchlistClick = () => {
    ...
    if (isInWatchlist) {
      ...
      addToWatchlist({ symbol, price, changePercent });
    }
  };
  ...
  if (!data) {
    return <Typography variant="h6" align="center">No data available for {symbol}</Typography>;
  }
    ...
    <Box sx={{ p: 4, backgroundColor: "#f4f6f8", minHeight: "100vh" }}>
      ...
      <Card elevation={4} sx={{ borderRadius: 3, overflow: "hidden", maxWidth: 900, mx: "auto" }}>
        <Box sx={{ backgroundColor: "#1976d2", color: "white", p: 2 }}>
          <Typography variant="h5" fontWeight="bold">{data.symbol || symbol}</Typography>
          <Typography variant="subtitle2">Latest Trading Data</Typography>
        </Box>
        <CardContent>
          <Grid container spacing={2}>
            <Grid item xs={6}><Typography>Open: ${data.open}</Typography></Grid>
            <Grid item xs={6}><Typography>High: ${data.high}</Typography></Grid>
            <Grid item xs={6}><Typography>Low: ${data.low}</Typography></Grid>
            <Grid item xs={6}><Typography>Price: ${data.close}</Typography></Grid>
            <Grid item xs={6}><Typography>Volume: {data.volume || "N/A"}</Typography></Grid>
            <Grid item xs={6}>
              <Typography sx={{ color: parseFloat(data.change) >= 0 ? "green" : "red" }}>
